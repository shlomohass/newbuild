...
class Trace {
    ...
    public static function add_trace($op, $class = 'unknown', $var_catch = array()) {
        ...
        if (is_array($var_catch) && count($var_catch) > 0) {
             foreach ($var_catch as $var_name => $data) {
    ...
    public static function expose_trace() {
        ...
        $buildHtml = array();
        //Main Wrapper:
        $buildHtml[] = "<div class='iid_debuger_view' ".$styles['wrapper'].">";
        //Header:
        $buildHtml[] = self::get_expose_header('DEBUGER CONSOLE');
        //Content wrapper:
        $buildHtml[] = "<div ".$styles['toggle']." class='iid_debuger_content_wrapper'><div class='iid_debuger_content' ".$styles['content'].">";
        //General:
    ...
    private static function get_expose_script() {
        ...
        $buildHtml .= " document.querySelector('.iid_debuger_header').addEventListener('click', function(e) { toggleSlide(document.querySelector('.iid_debuger_content_wrapper')); }, false);</script>";
        return $buildHtml;
    }
    private static function get_expose_header($header = '') {
        $styles = self::get_expose_styles();
        return "<div class='iid_debuger_header'".
               $styles['header'].
    ...
    private static function get_expose_styles() {
        return array(
            ...
            'content'       => "style='max-height: 400px; overflow-y: scroll; overflow-x:auto; padding: 0; margin: 0; display: block;'",
